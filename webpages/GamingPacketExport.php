<?php
    ...
    if (!$result=mysql_query($query,$link)) {
    	...
    	$title="Gaming Participant Schedule Export for Packets";
    	staff_header($title);
...
EOD;
    if (!$result=mysql_query($query,$link)) {
    	require_once('StaffHeader.php');
    	require_once('StaffFooter.php');
    	$title="Send CSV file of Gaming Packet Export";
    	staff_header($title);
    	$message=$query."<BR>Error querying database. Unable to continue.<BR>";
        echo "<P class\"errmsg\">".$message."\n";
        staff_footer();
    ...
    header('Content-disposition: attachment; filename=gaming_packet_export.csv');
