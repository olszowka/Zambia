...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="/">
        ...
        <div class="alert-block" id="resultBoxDIV">
            <div class="beforeResult">Result messages will appear here.</div>
        </div>
        <div class="row-fluid">
            <div class="span12">
                <form name="partform" class="form-horizontal" method="POST" action="SubmitMyContact.php">
                    <!-- Put all hidden inputs in this container so they don't mess up css styling of other elements -->
                    <div class="hidden-input-container">
                        <xsl:if test="not(/doc/options/@enable_share_email_question)">
                            <input name="share_email" type="hidden" value="{$share_email}"/>
                        </xsl:if>
                        <xsl:if test="not(/doc/options/@enable_use_photo_question)">
                            <input name="use_photo" type="hidden" value="{$use_photo}"/>
                        </xsl:if>
                        <xsl:if test="not(/doc/options/@enable_bestway_question)">
                    ...
                    <fieldset>
                        <div class="control-group">
                            <label for="interested" class="control-label nowidth">I am interested and able to
                                participate in programming for <xsl:value-of select="$conName"/>:
                            </label>
                            <div class="controls">
                                <select id="interested" name="interested" class="yesno span2"
                                        onchange="myProfile.anyChange('interested');"
                        ...
                        <xsl:if test="/doc/options/@enable_share_email_question">
                            <div class="control-group">
                                <label for="share_email" class="control-label nowidth">I give permission for
                                    <xsl:value-of select="$conName"/> to share my email address with other
                                    participants:
                                </label>
                                <div class="controls">
                                    <select id="share_email" name="share_email" class="span2"
                                            onchange="myProfile.anyChange('share_email')"
                        ...
                        <xsl:if test="/doc/options/@enable_use_photo_question">
                            <div class="control-group">
                                <label for="use_photo" class="control-label">I give permission for
                                    <xsl:value-of select="$conName"/> to photograph me while I am on panels and to
                                    use those images in the promotion of the convention:
                                    <xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text>
                                </label>
                                <div class="controls">
                                    <select id="use_photo" name="use_photo" class="span2"
                                            onchange="myProfile.anyChange('use_photo')"
                        ...
                        <xsl:if test="/doc/options/@enable_bestway_question">
                                ...
                                <div class="radioNlabel">
                                    <span class="radio">
                                        <input name="bestway" id="bwemailRB" value="Email" type="radio"
                                               onchange="myProfile.anyChange('bestway')"
                    ...
                    <p>Leave blank not to change your password.</p>
                    <fieldset>
                        <div id="passGroup">
                            <div class="control-group">
                                ...
                                <div class="controls">
                                    <input type="password" size="10" name="password" id="password"
                                           onchange="myProfile.anyChange('password')"
                                           onkeyup="myProfile.anyChange('password')"/>
                                </div>
                            </div>
                            <div class="control-group">
                                ...
                                <div class="controls">
                                    <input type="password" size="10" name="cpassword" id="cpassword"
                                           onchange="myProfile.anyChange('cpassword')"
                                           onkeyup="myProfile.anyChange('cpassword')"/>
                                    <span id="badPassword" class="help-inline"><xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text>Passwords
                                        don't match!
                                    </span>
                                </div>
                            </div>
                    ...
                    <div class="form-section-header">Information for Publication</div>
                    <div class="control-group">
                        ...
                        <label for="pubsname" class="control-label nowidth">Your name as you wish to have it
                            published:
                        </label>
                        <div class="controls">
                            <input type="text" size="20" name="pubsname"
                                   value="{/doc/query[@queryName='participant_info']/row/@pubsname}"
                                   id="pubsname" onchange="myProfile.anyChange('pubsname')"
                                   onkeyup="myProfile.anyChange('pubsname')"
                    ...
                    <div class="control-group" id="bioGroup">
                        <label for="bio">Your biography (<xsl:value-of select="/doc/options/@maxBioLen"/> characters or
                            fewer including spaces):
                        </label>
                        <div>
                            <textarea class="span12" rows="5" cols="72" name="bio" id="bioTXTA"
                                      onchange="myProfile.anyChange('bioTXTA')"
                    ...
                    <xsl:if test="/doc/query[@queryName='credentials']/row">
                        <div class="form-section-header">Professional Qualifications</div>
                        <div class="control-group">
                    ...
                    <button class="btn btn-primary" type="button" name="submitBTN" id="submitBTN"
                            data-loading-text="Updating..." onclick="myProfile.updateBUTN();">Update
                    </button>
                    <div class="form-section-header">Data from Registration System</div>
                    <fieldset>
                        <div class="row-fluid">
                            ...
                            <div class="pull-left span7">
                                <p class="congo_table">Please confirm your contact information. If it is not
                                    correct, please log into <xsl:value-of select="$conName"/>'s
                                    <a href="{$regURL}" target="_blank">on-line registration system</a>
                                    and correct it there. Please note that the password there is
                                    <span style="font-weight: bold">not the same</span> as the one you use
                                    in Zambia. This data is downloaded periodically from the registration database,
                                    and should be correct within an hour.
                                </p>
                            </div>
                        </div>
                    </fieldset>
