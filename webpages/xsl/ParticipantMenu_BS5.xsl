...
<xsl:stylesheet version="1.1" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template match="/">
    <nav id="participantNav" class="navbar navbar-expand-lg navbar-dark bg-dark px-4">
      ...
      <div class="collapse navbar-collapse flex-wrap" id="navbarSupportedContent">
        <ul class="navbar-nav">
          ...
          <xsl:if test="$PARTICIPANT_PHOTOS = '1' and /doc/query[@queryname='permission_set']/row[@permatomtag='photos']">
            <li class="nav-item py-0">
              <a class="nav-link py-1 pe-3" href="my_photo.php">Photo</a>
            </li>
          </xsl:if>
          <xsl:if test="$survey and count(/doc/query[@queryname='permission_set']/row[@permatomtag='survey']) > 0">
            <li class="nav-item py-0">
              <a class="nav-link py-1 pe-3" href="PartSurvey.php">Survey</a>
            </li>
          </xsl:if>
          <xsl:if test="/doc/query[@queryname='permission_set']/row[@permatomtag='my_availability']">
            <li class="nav-item py-0">
              <a class="nav-link py-1 pe-3" href="my_sched_constr.php">Availability</a>
            </li>
          </xsl:if>
          ...
          <xsl:if test="/doc/query[@queryname='permission_set']/row[@permatomtag='my_panel_interests']">
            <li class="nav-item py-0">
              <a class="nav-link py-1 pe-3" href="PartPanelInterests.php">Session Interests</a>
            </li>
          </xsl:if>
          <xsl:if test="/doc/query[@queryname='permission_set']/row[@permatomtag='my_schedule']">
            <li class="nav-item py-0">
              <a class="nav-link py-1 pe-3" href="MySchedule.php">My Schedule</a>
            </li>
          </xsl:if>
          <xsl:if test="/doc/query[@queryname='permission_set']/row[@permatomtag='BrainstormSubmit']">
            <li class="nav-item py-0 ms-4">
              <a class="nav-link py-1 pe-3" href="BrainstormWelcome.php">Suggest a Session</a>
            </li>
          </xsl:if>
        </ul>
