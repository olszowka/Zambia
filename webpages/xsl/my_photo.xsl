...
<!--
  ...
  Copyright (c) 2011-2026 Peter Olszowka. All rights reserved.
                ...
                <xsl:if test="$enablePhotos = 1">
                    <div class="row mt-4">
                        <div class="col-sm-5 card alert-secondary">
                            <input type="file" id="chooseFileName" name="chooseFileName"
                                   accept="image/png, image/jpeg, image/jpg" style="display: none"/>
                            <p class="card-title">
                                Upload Photo: Drag/Drop file or
                                <button type="button" class="btn btn-secondary btn-sm" id="uploadPhoto">Choose File
                                </button>
                            </p>
                            <div class="card-body" id="photoUploadArea"
                                ...
                                <img class="upload-image"
                                     style="width: 400px; height: 400px; object-fit: scale-down; margin-top:0; margin-right: auto; margin-left: auto;"
                                     id="uploadedPhoto">
                            ...
                            <div class="btn-group" role="group" aria-label="crop actions">
                                <button type="button" class="btn btn-primary btn-sm" id="crop" style="display: none;"
                                        onClick="myPhoto.crop();">Crop
                                </button>
                                <button type="button" class="btn btn-primary btn-sm" id="save_crop"
                                        style="display: none; margin-right: 10px;" onClick="myPhoto.savecrop();">Save
                                    Crop
                                </button>
                                <button type="button" class="btn btn-secondary btn-sm" id="rotate_left"
                                        style="display: none; margin-right: 10px;" onClick="myPhoto.rotate(90);">Rotate
                                    Left
                                </button>
                                <button type="button" class="btn btn-secondary btn-sm" id="rotate_right"
                                        style="display: none; margin-right: 10px;" onClick="myPhoto.rotate(-90);">Rotate
                                    Right
                                </button>
                                <button type="button" class="btn btn-warning btn-sm" id="cancel_crop"
                                        style="display: none" onClick="myPhoto.cancelcrop();">Cancel Crop
                                </button>
                            </div>
                        </div>
                        <div class="col-sm-1"/>
                            ...
                            <div class="card-body">
                                <img class="approved-image"
                                    ...
                                    <xsl:attribute name="src">
                                        ...
                                        <xsl:choose>
                                            <xsl:when
                                                    test="/doc/query[@queryName='participant_info']/row/@approvedphotofilename != ''">
                                                <xsl:value-of
                                                        select="/doc/query[@queryName='participant_info']/row/@approvedphotofilename"/>
                                            </xsl:when>
                                            <xsl:otherwise>
                                                <xsl:value-of select="$defaultPhotoName"/>
                                            </xsl:otherwise>
                    ...
                    <div class="row">
                        <div class="col-sm-5">
                            <input type="hidden">
                                <xsl:attribute name="value">
                                    <xsl:value-of
                                            select="/doc/query[@queryName='participant_info']/row/@photouploadstatus"/>
                                </xsl:attribute>
                            </input>
                            ...
                            <span id="uploadedPhotoStatus">
                                <xsl:choose>
                                    <xsl:when
                                            test="/doc/query[@queryName='participant_info']/row/@photouploadstatus = '-1'">
                                        No Photo Uploaded
                                    </xsl:when>
                                    <xsl:otherwise>
                                        <xsl:value-of
                                                select="/doc/query[@queryName='participant_info']/row/@statustext"/>
                                    </xsl:otherwise>
                                </xsl:choose>
                            </span>
                        </div>
                            ...
                            <div class="col-sm-5">
                                ...
                                <span id="DeniallReason">
                                    ...
                                    <xsl:if test="/doc/query[@queryName='participant_info']/row/@photodenialreasonothertext != ''">
                                        <xsl:text> (</xsl:text>
                                        <xsl:value-of
                                                select="/doc/query[@queryName='participant_info']/row/@photodenialreasonothertext"/>
                                        <xsl:text>)</xsl:text>
                                    </xsl:if>
                                </span>
                            </div>
                        ...
                        <div class="col-sm-5">
                            <div class="btn-group" role="group" aria-label="Photo Update/Delete actions">
                                <button type="button" class="btn btn-danger btn-sm" id="deleteUploadPhoto"
                                        onClick="myPhoto.deleteuploadedphoto();">
                                    <xsl:if test="string-length(/doc/query[@queryName='participant_info']/row/@uploadedphotofilename) = 0">
                                        <xsl:attribute name="style">display: none;</xsl:attribute>
                                    </xsl:if>
                                    Delete Uploaded Photo
                                </button>
                                <button type="button" class="btn btn-primary btn-sm" id="updateUploadPhoto"
                                        style="display: none;" onClick="myPhoto.starttransfer();">
                        ...
                        <xsl:if test="/doc/query[@queryName='participant_info']/row/@approvedphotofilename != ''">
                            ...
                            <div class="col-sm-5">
                                <button type="button" class="btn btn-danger btn-sm" id="deleteApprovedPhoto"
                                        onClick="myPhoto.deleteapprovedphoto();">
                    ...
                    <div class="row mt-1">
                        <div class="col-sm-12">
                            <xsl:choose>
                                <xsl:when test="string-length($photoNote) > 0">
                                    <xsl:value-of select="$photoNote" disable-output-escaping="yes"/>
                                </xsl:when>
                                <xsl:otherwise>
                                    <p>Note: Photos should be of type JPEG (.jpg) or PNG (.png), 1 MB or less in size and should be about 800x800. If you need to optimize your photo's file size we suggest "Save For Web".</p>
