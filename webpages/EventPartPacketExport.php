<?php
    ...
    if (!$result=mysql_query($query,$link)) {
    	...
    	$title="Event Participant Schedule Export for Packets";
    	staff_header($title);
...
EOD;
    if (!$result=mysql_query($query,$link)) {
    	require_once('StaffHeader.php');
    	require_once('StaffFooter.php');
    	$title="Send CSV file of Event Packet Export";
    	staff_header($title);
    	$message=$query."<BR>Error querying database. Unable to continue.<BR>";
        echo "<P class\"errmsg\">".$message."\n";
        staff_footer();
    ...
    if (mysql_num_rows($result)==0) {
    	require_once('StaffHeader.php');
    	require_once('StaffFooter.php');
    	$title="Send CSV file of Event Participant Packet Export";
    	staff_header($title);
    ...
    header('Content-disposition: attachment; filename=event_participant_packet_merge.csv');
