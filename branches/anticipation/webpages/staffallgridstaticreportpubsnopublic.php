<?php
    ...
    global $link;
    $_SESSION['return_to_page']="staffallgridstaticreportpubsnopublic.php";
    function topofpage() {
        staff_header($title);
...
EOD;
	...
	$s2p_data = mysql_query("select sessionid, pubsno from Sessions");
	$s2p = array();
	while ($s2p_row = mysql_fetch_array($s2p_data))
		$s2p[$s2p_row['sessionid']] = $s2p_row['pubsno'];
		
	$s2p_data = mysql_query("select sessionid, pubstatusid from Sessions");
	$stat2p = array();
	while ($s2p_row = mysql_fetch_array($s2p_data))
		$stat2p[$s2p_row['sessionid']] = $s2p_row['pubstatusid'];		
    ...
    for ($i=1; $i<=$rows; $i++) {
        ...
        for ($j=1; $j<=$rooms; $j++) {
			...
			{
            if ($x!="") {
                    echo sprintf("(<A HREF=\"StaffAssignParticipants.php?selsess=%s\">".htmlspecialchars($s2p[$x])."</A>) ",$x);
