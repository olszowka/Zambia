...
CREATE TABLE `PermissionAtoms` (
    ...
    `permatomname` VARCHAR(50) DEFAULT NULL,
...
CREATE TABLE `Permissions` (
    ...
    `badgeid_norm` VARCHAR(15) GENERATED ALWAYS AS (IFNULL(`badgeid`, '')) VIRTUAL,
    PRIMARY KEY (`permissionid`),
    UNIQUE KEY `unique1` (`permatomid`,`phaseid`,`permroleid`,`badgeid_norm`),
    KEY `FK_PermAtoms` (`permatomid`),
...
INSERT INTO `PermissionAtoms`
    ...
    VALUES
    (1, 'Staff', 'Staff General', NULL, 'renderWelcome', '(Admin & Staff) Enables staff login, menu, welcome page, etc.', 10),
    (2, 'Administrator', 'Administrator General', NULL, 'many', '(Admin) Configure survey, custom text, etc.', 20),
    (3, 'Participant', 'Participant General', NULL, 'many', '(Participant) Login as Participant', 410),
    (4, 'EditBio', 'Edit Published Items', NULL, 'renderMyContact', '(Participant) Allow edit of biography and name for publications.', 430),
    (5, 'my_availability', 'Availability', NULL, 'Participant Menu', '(Participant) Availability page', 460),
    (6, 'search_panels', 'Search Sessions', NULL, 'Participant Menu', '(Participant) Access Search Sessions page', 500),
    (7, 'my_panel_interests', 'Session Interests', NULL, 'Participant Menu', '(Participant) Access Session Interests (Panel Interests) page', 510),
    (8, 'my_schedule', 'My Schedule', NULL, 'Participant Menu', '(Participant) Access My Schedule page', 520),
    (9, 'my_suggestions_write', 'My Suggestions Write', NULL, 'MySuggestions', '(Participant) Edit My Suggestions page', 470),
    (10, 'my_gen_int_write', 'General Interests Write', NULL, 'MyGeneralInterests', '(Participant) Edit General Interests page', 450),
    (11, 'BrainstormSubmit', 'Brainstorm Submit Session', NULL, 'EditCreateSession', '(Participant) Can view Brainstorm section and submit brainstorm session', 550),
    (12, 'BS_sear_sess', 'Brainstorm Search Session', NULL, 'SearchSessions', '(Participant) Can view sessions from Brainstorm section', 560),
    (13, 'public_login', 'Brainstorm Public Login', NULL, 'Login', '(Other) Not used', 570),
    ...
    (21, 'EditUserPermRoles', 'Edit User Permission Roles', NULL, 'Admin Participants', '(Staff) Control all user permission roles. Also used on create user page.', 110),
    (22, 'EditUserPermRoles', 'Edit User Permission Roles (Participant)', 4, 'Admin Participants', '(Staff) Control participant user permission role only. Also used on create user page.', 130),
    (23, 'EditUserPermRoles', 'Edit User Permission Roles (Staff)', 3, 'Admin Participants', '(Staff) Control staff user permission role only. Also used on create user page.', 120),
    (24, 'edit_my_contact', 'Edit Profile', NULL, 'profile/my_contact', '(Staff & Participant) All login roles should have it to change their passwords.', 420),
    (25, 'general_interests', 'General Interests', NULL, 'Participant Menu', '(Participant) Access General Interests page', 440),
    (26, 'EditAnyConfigurationTable', 'Edit Any Configuration Table', NULL, 'Edit Configuration Tables', '(Admin) Global permission to edit configuration tables', 50),
    (27, 'photos', 'Photo', NULL, 'Participant Menu', '(Participant) Access Photo page', 480),
    (28, 'survey', 'Survey', NULL, 'Participant Menu', '(Participant) Access Survey page', 490),
    (29, 'AdminPhotos', 'Approve Photos', NULL, 'Administer Photos', '(Staff) Approve and deny participant photos', 140),
    (30, 'edit_participant_tags', 'Edit Participant Tags', NULL, 'Admin Participants', '(Staff) Change a participant\'s tags', 150),
    (31, 'edit_participant_responses', 'Edit Participant Responses', NULL, 'many', '(Staff) edit others\' survey responses, etc.', 160),
    (32, 'declined_participant', 'Declined Participant', NULL, 'many', '(Other) Can view only Declined Participant page', 540),
    (33, 'ConfigurePermissions', 'Configure Permissions', NULL, 'ConfigurePermissions', '(Admin) Use Configure Permissions page to edit the Permissions, Phases, and PermissionRoles', 60),
    (2000, 'ce_BioEditStatuses', 'Configure BioEditStatuses', NULL, 'Edit Configuration Tables', '(Admin) This table is not currently used', 170),
    (2001, 'ce_Credentials', 'Configure Credentials', NULL, 'Edit Configuration Tables', '(Admin) Characteristic of participant to distinguish author, editor, etc.', 180),
    (2002, 'ce_Roles', 'Configure Roles', NULL, 'Edit Configuration Tables', '(Admin) Characteristic of participant such as panelist, lecturer, etc.', 190),
    (2003, 'ce_KidsCategories', 'Configure KidsCategories', NULL, 'Edit Configuration Tables', '(Admin) Characteristic of session to descript age appropriateness', 200),
    (2004, 'ce_LanguageStatuses', 'Configure LanguageStatuses', NULL, 'Edit Configuration Tables', '(Admin) This table is not currently used', 210),
    (2005, 'ce_PubStatuses', 'Configure PubStatuses', NULL, 'Edit Configuration Tables', '(Admin) Whether a session will be publicized', 220),
    (2006, 'ce_SessionStatuses', 'Configure SessionStatuses', NULL, 'Edit Configuration Tables', '(Admin) Steps in workflow for a session', 230),
    (2007, 'ce_Divisions', 'Configure Divisions', NULL, 'Edit Configuration Tables', '(Admin) Characteristic of session to indicate responsible team within convention', 240),
    (2008, 'ce_RegTypes', 'Configure RegTypes', NULL, 'Edit Configuration Tables', '(Admin) Provide a description for a code from the registration system', 250),
    (2009, 'ce_Tags', 'Configure Tags', NULL, 'Edit Configuration Tables', '(Admin) Characteristic of session to categorize by topic; may be published', 260),
    (2010, 'ce_Times', 'Configure Times', NULL, 'Edit Configuration Tables', '(Admin) Time slots which appear on the participant availability page', 270),
    (2011, 'ce_Tracks', 'Configure Tracks', NULL, 'Edit Configuration Tables', '(Admin) Characteristic of session to categorize by topic; may be published', 280),
    (2012, 'ce_Types', 'Configure Types', NULL, 'Edit Configuration Tables', '(Admin) Characteristic of session to categorize by activity; may be published', 290),
    (2013, 'ce_Rooms', 'Configure Rooms', NULL, 'Edit Configuration Tables', '(Admin) Real or virtual location of session', 300),
    (2014, 'ce_RoomSets', 'Configure RoomSets', NULL, 'Edit Configuration Tables', '(Admin) Arrangement of furniture in a room', 310),
    (2015, 'ce_RoomHasSet', 'Configure RoomHasSet', NULL, 'Edit Configuration Tables', '(Admin) Which furniture arrangement fits in which room and what the capacity will be', 320),
    (2016, 'ce_Features', 'Configure Features', NULL, 'Edit Configuration Tables', '(Admin) Characteristic of session describing aspects of the room it requires', 330),
    (2017, 'ce_Services', 'Configure Services', NULL, 'Edit Configuration Tables', '(Admin) Characteristic of session describing things staff must provide for it', 340),
    (2018, 'ce_EmailFrom', 'Configure EmailFrom', NULL, 'Edit Configuration Tables', '(Admin) From addresses for email messages', 350),
    (2019, 'ce_EmailTo', 'Configure EmailTo', NULL, 'Edit Configuration Tables', '(Admin) Queries to select whom to send email messages to', 360),
    (2020, 'ce_EmailCC', 'Configure EmailCC', NULL, 'Edit Configuration Tables', '(Admin) CC addresses for email message', 370),
    (2021, 'ce_PhotoDenialReasons', 'Configure PhotoDenialReasons', NULL, 'Edit Configuration Tables', '(Admin) Reasons for denying a photo', 380),
    (2022, 'ce_ParticipantTags', 'Configure ParticipantTags', NULL, 'Edit Configuration Tables', '(Admin) Miscellaneous characteristic of a particpant', 390),
    (10001, 'ConTrollImportUsers', 'ConTroll Import Users', NULL, 'Import Reg User', '(Staff) Enables importing users from ConTroll', 400);
